...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: "إعدادات الشات بوت", href: "/dashboard/settings", icon: Settings },
  ];
  ...
  const getPlanBadgeColor = (tier: string) => {
    switch (tier) {
      ...
      case "business": return "bg-amber-500/10 text-amber-400 border border-amber-500/20";
      ...
      <aside className="w-72 bg-slate-900 border-l border-slate-800 flex flex-col z-20">
        <div className="p-6 border-b border-slate-800 flex items-center gap-3 shrink-0">
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-500 mt-1 block">منصة الشات بوت الذكي</span>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50 shrink-0">
          <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800 mb-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-500">الباقة الحالية</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${getPlanBadgeColor((user as any).subscription_tier || "free")}`}>
                {getPlanName((user as any).subscription_tier || "free")}
              </span>
            </div>
